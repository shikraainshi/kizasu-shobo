        ...
        <div className="bg-background">
          ...
          <header className="mb-16 border-b border-accent/10 pb-12">
            <div className="flex items-center gap-6 mb-8">
              ...
              <span className={`text-[10px] tracking-[0.2em] border px-4 py-1.5 font-bold font-serif ${getCategoryColor(newsItem.category)}`}>
                {newsItem.category}
              </span>
            </div>
          ...
          {/* 3. Main Content */}
          <section className="max-w-none font-serif text-base leading-relaxed text-foreground/80 mb-20 whitespace-pre-wrap space-y-8">
            <div>{newsItem.content}</div>
            
            <div className="flex flex-col gap-0.5">
              {[
                { url: newsItem.relatedUrl1, label: newsItem.urlLabel1 },
                { url: newsItem.relatedUrl2, label: newsItem.urlLabel2 },
              ].map(({ url, label }, i) =>
                url ? (
                  <a
                    key={i}
                  ...
                  >
                    <span className="text-sm break-all">{label || url}</span>
